...
def datetime_12hr_filter(value):
    if value is None:
        return 'N/A'  # or return '' for an empty string, or any other default message you prefer
    if isinstance(value, str):
        value = datetime.strptime(value, '%Y-%m-%d %H:%M:%S')
    return value.strftime('%Y-%m-%d %I:%M:%S %p')

# Register the filter
app.jinja_env.filters['datetime_12hr'] = datetime_12hr_filter
...
def login():
    ...
    if request.method == "POST":
        ...
        connection = create_connection("localhost", "root", "", "salesdashboard")
...
def home():
    ...
    if connection:
        ...
        try:
            ...
            cursor.execute("""
                SELECT agent_name, SUM(points) AS sales_points
                FROM orders
            ...
            cursor.execute("""
                SELECT MONTH(time) AS month, COUNT(*) AS sales_count, SUM(points) AS sales_points
                FROM orders
...
def add_order():
    ...
    # Define the points for each package
    package_points = {
        "Silver": 1,
        "Gold": 2,
        "Platinum": 3,
    ...
    # Get the points based on the package
    points = package_points.get(order_data["package"], 0)
    ...
    if connection:
        ...
        cursor.execute(
            "INSERT INTO orders (order_id, time, status, customer_name, customer_phone, customer_vin, package, agent_name, approve, screenshot, points) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
            (
                order_data["order_id"],
                order_time,
                order_data["status"],
...
def number():
    ...
    if connection:
        ...
        # Add separators in numbers list
        prev_time = None
        separated_numbers = []
        for number in numbers:
            assigned_time = number['assigned_at'].strftime('%Y-%m-%d %H:%M:%S')
            if prev_time and prev_time != assigned_time:
                separated_numbers.append({'separator': True})
            separated_numbers.append(number)
            prev_time = assigned_time
        numbers = separated_numbers
...
@app.route("/delete_remaining_numbers", methods=["POST"])
def delete_remaining_numbers():
    if "user_id" not in session:
        return redirect(url_for("index"))
    ...
    if connection:
        try:
            cursor = connection.cursor()
            cursor.execute("DELETE FROM unique_numbers WHERE assigned = FALSE")
            connection.commit()
            session["message"] = "Remaining numbers deleted successfully."
        except Error as e:
            connection.rollback()
            session["message"] = f"Failed to delete remaining numbers: {e}"
        finally:
            cursor.close()
            connection.close()
    else:
